...
In our own work on identifying \emph{socio-technical} factors with fault prediction capability, we found that synchronizing private development lines often leads to merge conflicts, which in turn is a significant factor when we model the bugginess of the lines of code \cite{ahmedempirical}. Moreover, we found that code involved in merge conflicts contain almost 3 times more design issues (one indicator of code quality that also has association with bug-proneness) than non-conflicting code. The primary idea behind this project is that \emph{socio-technical} factors have high association with fault proneness, so automated test cases generated using them as coverage criteria, in conjunction with other traditional factors, should provide more effective test cases with  higher defect identification capability.
